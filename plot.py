...
def parse_file(file_name, old=False):
    file_path = file_name
    size_list = []
    gflop_list = []
    ...
    print(" ".join([str(i) for i in size_list]))
    print(" ".join([str(i) for i in gflop_list]))
    global test_list, old_list, mkl_list
    if old and len(gflop_list) == 30:
        old_list = gflop_list
    elif "mkl" in file_name:
        mkl_list = gflop_list
    else:
        test_list = gflop_list
...
def show_plot():
    ...
    plt.plot(N_list, mkl_list, label="mkl", marker="*")
    plt.plot(N_list, test_list, label="test", marker="3")
    plt.plot(N_list, blas_list, label="blas", marker="*")
    plt.plot(N_list, old_list, label="old", marker="4")
