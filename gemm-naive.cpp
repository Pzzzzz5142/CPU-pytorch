...
{
    static float packA[M_KERNEL_SIZE * K_BLOCK_SIZE] __attribute__((aligned(32)));
    static float packB[N_BLOCK_SIZE * K_BLOCK_SIZE] __attribute__((aligned(32)));
    // static float nextPackA[M_KERNEL_SIZE * K_BLOCK_SIZE] __attribute__((aligned(32)));
    // static float nextPackB[N_BLOCK_SIZE * K_BLOCK_SIZE] __attribute__((aligned(32)));
    for (int i = 0; i < m; i += M_KERNEL_SIZE)
    {
        ...
        for (int j = 0; j < n; j += N_KERNEL_SIZE)
        {
            ...
            addDot_asm_6x16(k, newN, packA, M_KERNEL_SIZE, packB + (j * k), N_KERNEL_SIZE, &C(i, j));
        }
    }
