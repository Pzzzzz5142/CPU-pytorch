...
{
    auto linear = std::make_shared<CPU_Packed_Linear<float>>(in_features, out_features, bias);
...
torch::Tensor cpu_linear_forward(int packed_linear_id, const torch::Tensor &input, const torch::optional<torch::Tensor> &bias_opt = torch::nullopt)
{
    std::shared_ptr<CPU_Packed_Linear<float>> lin = std::static_pointer_cast<CPU_Packed_Linear<float>>(s_linears[packed_linear_id]);
    ...
    if (bias_opt)
    {
        ...
        gemm_compute(input_col, n, k, input_ptr, lin->data_ptr(), output_ptr, bias_opt ? 1 : 0);
